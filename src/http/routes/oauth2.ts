import { getDiscordConnections, getDiscordToken, getDiscordUser } from "@/lib/discord/DiscordAPI.js";
...
const routes = async (fastify: FastifyInstance): Promise<void> => {
  fastify.get("/oauth2", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await handleDiscordConnectionsTx(tx, discordUser.id, discordConnections);
      });
