...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option
      ...
      .setDescriptionLocalizations(getAllLocalizedTranslations("banSlashCommandUserIdOption"))
      .setRequired(true),
  )
  .addStringOption((option) =>
    option
      ...
      .setDescriptionLocalizations(getAllLocalizedTranslations("banSlashCommandReason"))
      .setRequired(false),
  )
...
async function tryBan(interaction: ChatInputCommandInteraction, userID: string, reason: string): Promise<void> {
  try {
    ...
    if (banErrorKey) {
      return replyEphemeral(interaction, banErrorKey as TranslationKeys, getUserLang(interaction.locale));
    }
    ...
    return replyEphemeral(interaction, "successBan", getUserLang(interaction.locale), { userID, reason });
  } catch {
    try {
      ...
      return replyEphemeral(interaction, "errorBan", getUserLang(interaction.locale));
    }
  }
}
